...
const normalizeValueOptions = (valueOptions: any[] | undefined): NormalizedOption[] => {
      ...
      if (option.reporting_date) {
        ...
        const dateValue =
          formattedDate.length === 8
            ? `${formattedDate.slice(0, 4)}-${formattedDate.slice(4, 6)}-${formattedDate.slice(6, 8)}`
            : formattedDate;
        return {
          value: dateValue,
          label: dateValue,
...
const getValidationErrors = (
  ...
  return validationResults.filter((result) => {
    const rowMatch = row.id ? result.row_id === row.id.toString() : result.row_id === 'unsaved';
    const columnMatch = result.column_name.toLowerCase() === columnName.toLowerCase();
...
export const MetadataTableCell: React.FC<MetadataTableCellProps> = ({
  ...
  const validationErrors = getValidationErrors(row, item.code, validationResults);
  ...
  const renderInput = () => {
    if (normalizedOptions.length > 0) {
      return (
        <Select
          value={currentValue}
          onValueChange={(value) => handleCellChange(rowIndex, item.code, value)}
    ...
    if (inputType === 'date' && item.datatype === 'GregorianDay') {
      ...
      return (
        <DatePicker
          value={dateValue}
          onChange={(newValue) => handleCellChange(rowIndex, item.code, newValue)}
          isValidDate={(date) => isValidDate(format(date, 'yyyy-MM-dd'), datePattern)}
    ...
    return (
      <Input
        type={inputType}
        value={currentValue}
        onChange={(e) => handleCellChange(rowIndex, item.code, e.target.value || null)}
        // disabled={item.is_system_generated}
        {...commonInputProps}
        data-validation-error={hasError}
      />
  ...
  return (
    <TableCell
      className={cn('relative px-4 py-2 text-sm', hasError && 'bg-red-50')}
      data-validation-error={hasError}
    >
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <AlertTriangle
                  className="absolute right-2 top-1/2 h-4 w-4 -translate-y-1/2 transform cursor-pointer text-red-500"
