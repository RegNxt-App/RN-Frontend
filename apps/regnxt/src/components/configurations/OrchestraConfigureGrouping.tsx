...
const OrchestraConfigureGrouping = () => {
  ...
  const {
    ...
    mutate: mutateGroups,
    isLoading,
  } = useSWR<Grouping>('/api/v1/groups/', orchestraBackendInstance);

  const handleCreateGroup = async (newGroup: Partial<Group>) => {
    try {
      await orchestraBackendInstance.post('/api/v1/groups/', newGroup);
      await mutateGroups();
  ...
  const handleUpdateGroup = async (updatedGroup: Group) => {
    try {
      await orchestraBackendInstance.put(`/api/v1/groups/${updatedGroup.code}/`, updatedGroup);
      await mutateGroups();
  ...
  const handleDeleteGroup = async (groupCode: string) => {
    try {
      await orchestraBackendInstance.delete(`/api/v1/groups/${groupCode}/`);
      await mutateGroups();
...
export default OrchestraConfigureGrouping;
