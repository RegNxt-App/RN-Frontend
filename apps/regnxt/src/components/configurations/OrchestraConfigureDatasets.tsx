...
import OrchestraDatasetFormModal from './OrchestraDatasetFormModal';
...
const OrchestraConfigureDatasets = () => {
  ...
  const {data: layers} = useSWR<Layers>('/api/v1/layers/', orchestraBackendInstance);
  const {data: frameworks} = useSWR<Frameworks>('/api/v1/frameworks/', orchestraBackendInstance);
  ...
  const handleCreateDataset = async (newDataset: Partial<Dataset>) => {
    try {
      await orchestraBackendInstance.post('/api/v1/datasets/', {
        ...newDataset,
  ...
  const handleUpdateDataset = async (updatedDataset: Dataset) => {
    ...
    try {
      await orchestraBackendInstance.put(`/api/v1/datasets/${updatedDataset.dataset_id}/`, updatedDataset);
      await mutateDatasets();
      toast({
        title: 'Success',
  ...
  const handleDeleteDataset = async () => {
    ...
    try {
      await orchestraBackendInstance.delete(`/api/v1/datasets/${deletingDatasetId}/`);
      await mutateDatasets();
      toast({
        title: 'Success',
  ...
  const handleCreateVersion = async (dataset: Dataset & Partial<DatasetVersion>) => {
    ...
    try {
      ...
      await orchestraBackendInstance.post(`/api/v1/datasets/${dataset.dataset_id}/create_version/`, payload);
  ...
  const handleUpdateVersion = async (version: DatasetVersion) => {
    try {
      await orchestraBackendInstance.put(
  ...
  const handleDeleteVersion = async (datasetId: number, versionId: number) => {
    try {
      await orchestraBackendInstance.delete(
  ...
  const handleConfigurationSave = async (configData: {is_visible: boolean; historization_type: string}) => {
    ...
    try {
      await orchestraBackendInstance.put(`/api/v1/datasets/${selectedDataset.dataset_id}/`, {
        ...selectedDataset,
        is_visible: configData.is_visible,
      });

      if (selectedVersion) {
        await orchestraBackendInstance.put(`/api/v1/datasets/${selectedDataset.dataset_id}/update-columns/`, {
...
export default OrchestraConfigureDatasets;
