...
export const fetchTableStructure = createAsyncThunk<ApiResponse[], number, {rejectValue: string}>(
  ...
  async (workbookId: number, {rejectWithValue}) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to fetch table structure';
...
export const fetchSheetData = createAsyncThunk<
  ...
  {rejectValue: string}
>('sheetData/fetchSheetData', async ({workbookId, sheetId}, {rejectWithValue}) => {
  try {
  ...
  reducers: {
    ...
    addChangedRows: (state, action: PayloadAction<ChangedRow[]>) => {
      action.payload.forEach((newRow) => {
        const existingRowIndex = state.changedRows.findIndex((row) => row.rowId === newRow.rowId);

        if (existingRowIndex !== -1) {
          state.changedRows[existingRowIndex] = {
            ...state.changedRows[existingRowIndex],
            updatedRow: newRow.updatedRow,
            changedCells: [...state.changedRows[existingRowIndex].changedCells, ...newRow.changedCells],
            timestamp: Date.now(),
          };
        } else {
    ...
    updateChangedRow: (state, action: PayloadAction<ChangedRow>) => {
      const index = state.changedRows.findIndex((row) => row.rowId === action.payload.rowId);
      if (index !== -1) {
    ...
    setDialogState: (state, action: PayloadAction<{isOpen: boolean; dialogType: string | null}>) => {
...
// Export selectors
export const selectChangedRows = (state: {sheetData: SheetDataState}) => state.sheetData.changedRows;
export const selectLastChangeTimestamp = (state: {sheetData: SheetDataState}) =>
  state.sheetData.lastChangeTimestamp;
export const selectTotalCounts = (state: {sheetData: SheetDataState}) => state.sheetData.totalCounts;
export const selectDialogState = (state: {sheetData: SheetDataState}) => state.sheetData.dialogState;
export const selectTableStructure = (state: {sheetData: SheetDataState}) => state.sheetData.tableStructure;
export const selectTableStructureLoading = (state: {sheetData: SheetDataState}) =>
  state.sheetData.tableStructureLoading;
export const selectStructureError = (state: {sheetData: SheetDataState}) => state.sheetData.structureError;
export const selectSelectedSheet = (state: RootState) => state.sheetData.selectedSheet;
