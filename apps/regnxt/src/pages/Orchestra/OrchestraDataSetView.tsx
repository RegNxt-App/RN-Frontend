...
const OrchestraDataSetView: React.FC = () => {
  ...
  const fetchTableData = useCallback(async () => {
    ...
    try {
      const [metadataResponse, dataResponse] = await Promise.all([
        orchestraBackendInstance.get(`/api/v1/datasets/${selectedTable.dataset_id}/version-columns/`, {
          params: {version_id: datasetVersion.dataset_version_id},
        }),
        orchestraBackendInstance.get(
  ...
  const handleValidate = useCallback(
    async (tableData: Record<string, string | null>[]) => {
      ...
      try {
        ...
        const response = await orchestraBackendInstance.post<ValidationResult[]>(
...
export default OrchestraDataSetView;
